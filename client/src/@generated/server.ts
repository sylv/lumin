...
export type Torrent = { id: number; hash: string; name: string; remote_id: number | null; magnet_uri: string; state: TorrentState; progress: number; seeds: number; peers: number; upload_speed: number; download_speed: number; ratio: number; error_message: string | null; eta_secs: number; size: number; category: string | null; files_created: boolean; created_at: number; updated_at: number; checked_at: number | null; finished_at: number | null }
...
delete_torrent: publicProcedure.input((value): DeleteTorrentInput => { throw new Error('Router should not be used') }).query((opts): void => { throw new Error('Router should not be used') })
});
